...
use crate::error::MlmfError;
use crate::loader::LoadedModel;
use crate::metadata::{MemoryRequirement, ModelMetadata};
...
impl ModelCache {
    ...
    pub fn insert(&self, cache_key: String, model: Arc<LoadedModel>) -> Result<(), MlmfError> {
    ...
    pub fn evict_lru(&self, count: usize) -> Result<usize, MlmfError> {
        let mut cache = self.cache.write().unwrap();
        let mut lru_order = self.lru_order.write().unwrap();
        let mut current_memory = self.current_memory_usage.write().unwrap();
    ...
    fn estimate_model_memory(&self, model: &LoadedModel) -> u64 {
        if let Some(metadata) = model.metadata() {
            if let Some(memory_req) = &metadata.memory_requirement {
                return memory_req.minimum_memory_mb as u64 * 1024 * 1024;
            }
        }

        // Fallback estimation based on parameter count
        // Rough estimate: 4 bytes per parameter for fp32, plus overhead
        let estimated_params = 1_000_000; // Default conservative estimate
        (estimated_params * 4 * 12 / 10) as u64 // 20% overhead
    }

    /// Handle memory pressure by evicting models
    fn handle_memory_pressure(&self) -> Result<(), MlmfError> {
