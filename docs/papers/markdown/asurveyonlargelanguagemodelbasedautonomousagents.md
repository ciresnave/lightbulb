...
Zhi-Yuan Chen, Jiakai Tang, Xu Chen((cid:66)), Yankai Lin((cid:66)), Wayne Xin Zhao,
...
papers published from January 2021 to August 2023. We assign different colors to represent various agent categories. For
...
ing upon this capability, there has been a growing

research area that employs LLMs as central con-
...
Furthermore, LLM-based agents can offer natural
...
plete various tasks effectively. Previously, these
...
2021-12022-12023-22023-42023-62023-8Time?Year-Month?NumberofPapers?cumulated?WebGPT2021-12Generative Agent2023-4Voyager 2023-5ToolBench 2023-7AgentSims 2023-8Tool AgentSimulation AgentGeneral AgentEmbodied AgentTALM2022-5HuggingGPT2023-3Game AgentCoT 2022-1Web AgentWebShop2022-7InnerMonologue2022-7DEPS2023-2Toolformer2023-2AutoGPT2023-3Assistant AgentAgentGPT2023-4RecAgent 2023-6GITM2023-5ToT2023-5MIND2WEB2023-6CO-LLM 2023-7ChatDev 2023-7Lei Wang et al. A Survey on Large Language Model based Autonomous Agents
...
ity to complete different tasks. Intuitively, the first
...
encompass basic information such as age, gender,

and career [20], as well as psychology information,
...
would like to design agents with different person-
...
�DemographicInformation�PersonalityInformation�SocialInformation�HandcraftingMethod�LLM-GenerationMethod�DatasetAlignmentMethodProfileActionMemoryPlanning�Task Completion�Communication�Memory Reading�Memory Writing�Memory Reflection�Unified Memory�Hybrid Memory�Languages�Embeddings �EnvironmentFeedback�Human Feedback�Model Feedback�Single-path Reasoning�Multi-path Reasoning�External PlannerGenerationStrategyProfileContentsMemoryOperationMemoryStructureMemoryFormatsPlanningw/oFeedbackPlanningw/FeedbackActionTargetActionProductionActionImpact�Tools�Databases�Lists�Self-Knowledge�Exploration�Memory Recollection�Environments�Internal StatesActionSpace�Plan Following�New ActionsLei Wang et al. A Survey on Large Language Model based Autonomous Agents
...
tools such as IPIP-NEO [26] and BFI [27].
[28]
studies the toxicity of the LLM output by manu-
ally prompting LLMs with different roles, such as
...
time and effort required to construct agent profiles,
...
consistent, reasonable, and effective manner. This
...
In this agent, the scene graphs and envi-
ning.
...
and narration of stories.
Its short-term memory
is built upon scene descriptions, monster informa-

tion, and previous summaries. DEPS [33] is also
...
ories. The short-term memory temporarily buffers
...
short-term memory to store different objects, which
...
memory or embedding memory. Different memory
...
enhances both retrieval and reading efficiency. For
instance, MemoryBank [39] encodes each memory
segment as an embedding vector and employs a
dual-tower dense retrieval model to efficiently re-
...
to manipulate memories efficiently and comprehen-
...
allows for efficient retrieval of memory records. By
...
flection. In the following, we introduce these opera-
...
(cid:16)
?srec(q, m) + ?srel(q, m) + ?simp(m)
(1)
where q is the query, for example, the task that
the agent should address or the context in which
the agent is situated. M is the set of all memories.
srec(�), srel(�) and simp(�) are the scoring functions for
...
It should be noted that simp only reflects the charac-
ters of the memory itself, thus it is unrelated to the
query q. ?, ? and ? are balancing parameters. By
assigning them with different values, one can obtain
various memory reading strategies. For example, by
setting ? = ? = 0, many studies [16, 30, 38, 41] only
consider the relevance score srel for memory read-
ing. By assigning ? = ? = ? = 1.0, [20] equally
...
agent to act more efficiently and rationally. During
...
it is full, people design different methods to delete
...
RET-LLM [41] uses a fixed-size buffer for memory,
...
vide agents with the capability to independently

summarize and infer more abstract, complex and
...
in the prompts. Zero-shot-CoT [46] enables LLMs

to generate task reasoning processes by prompt-
...
on Huggingface. Different from CoT and Zero-
...
PromptsLLMReasoning Step-1Reasoning Step-2Reasoning Step-nCoT?Zero-shotCotPromptsLLMReasoning Step-1Reasoning Step-2ReWOO?HuggingGPTLLMReasoning Step-nLLMPromptsLLMStep-1Step-2Step-nCoT-SCSingle-PathReasoningMulti-PathReasoningStep-1Step-2Step-nStep-1Step-2Step-nPromptsLLMToT?LMZSP?RAPStep-1Step-2Step-2Step-2Step-3Step-3Step-3Step-3Lei Wang et al. A Survey on Large Language Model based Autonomous Agents
...
planners. At each planning step, they first generate

multiple possible next steps, and then determine
...
are well-developed and employ efficient search algo-
...
an external planner, which efficiently determines
...
ternal low-level planner is employed to effectively
...
beginning is extremely difficult as it needs to con-
...
more adaptive to the environment. Voyager [38]
...
Voyager, Ghost [16] also incorporates feedback into
the reasoning and action taking processes. This
...
them to more effectively revise their plans. LLM-
...
the above cases, we can see, different types of feed-
...
vide feedback on the output and offer guidance on
...
erroneous and inefficient actions. ChatCoT [66]
...
ous studies, where the feedback is given as a scalar
...
powerful and capable of effectively addressing com-
...
Voyager [38] may explore unknown skills in their
...
sociated, the agent may take actions via different
...
pleted previously, the agent invokes the previously

successful actions to handle the current task directly.
In collaborative agents such as ChatDev [18] and
MetaGPT [23], different agents may communicate
...
lowing.
In this strategy, the agent takes actions
...
demonstrated to be effective in accomplishing a
...
tool APIs for training. API-Bank [70] offers a com-
...
employs SQL statements to query databases, facil-
...
and effectively. (1) Planning Capability. Previous
...
example, in ChatDev [18], different agents can dis-
...
potential feedback on the agent�s utterance.
(3)
...
GITM [16] and Voyager [38], the environments are
...
the agent can also change the agent itself, including

updating memories, forming new plans, acquiring
...
ample, in Voyager [38], once the agent has gathered
...
relying solely on the hardware is insufficient for
achieving effective task performance. This is be-
...
ViperGPT [75]
HuggingGPT [13]
Generative Agents [20]
LLM+P [58]
ChemCrow [76]
OpenAGI [74]
AutoGPT [82]
SCM [35]
Socially Alignment [83]
GITM [16]
Voyager [38]
Introspective Tips [84]
RET-LLM [41]
ChatDB [40]
S 3 [78]
ChatDev [18]
...
human values and preferences. Different from the
...
question, the central agent first generates initial re-
...
Illustration of transitions in strategies for acquiring model capabilities.
...
examples in the prompt. Similar techniques are also
...
ModelOutputInputDatasetParameter LearningThe era of machine learningParameter LearningThe era of large language modelPrompt EngineeringThe era of agentMechanism EngineeringPrompt EngineeringModel ParametersCapabilityModelCapabilityParameter LearningModelOutputCapabilityPromptsClassify the text into neutral, negative or positive. Text: I think the food was okay. Sentiment:NeutralAgent relevant PromptsOutputCrowd-sourcingTrial-and-Error20
...
back differ, the critic generates failure information,
...
capability of each agent is enhanced by understand-
...
which do not explicitly mention LLM-based agents

but are highly related to this area.

3 LLM-based Autonomous Agent Ap-
...
authors assign LLMs with different profiles, and
...
of harmful information, and so on [20, 34, 78, 80,

106�109]. For example, Social Simulacra [80] sim-
...
SocialScienceNaturalScienceEngineering�Psychology�Political Science and Economy�Social Simulation�Documentation and Data Management�Natural Science Experiment Assistant�Natural Science Education�Civil Engineering�Computer Science�Aerospace Engineering�Jurisprudence�Social Science�Research Assistant�Industrial Automation�Robotics & Embodied AISubjectiveEvaluation�HumanAnnotation�TuringTestObjectiveEvaluation�Evaluation Metric�Evaluation Protocol�Evaluation BenchmarkApplicationEvaluationLei Wang et al. A Survey on Large Language Model based Autonomous Agents
...
of literature, which requires a significant dedication

of time and human resources. LLM-based agents
...
the agent can efficiently query and utilize internet
...
programming. It offers many useful features, such
...
to open-ended questions and offer feedback.
...
In the field of computer science and software en-

gineering, LLM-based agents offer potential for
...
work demonstrates efficient and cost-effective gener-
...
code vulnerabilities. This approach effectively man-
ages the trade-off between accuracy and scalability.
...
tool based on LLMs, which can effectively identify
...
TE [101], Akata et al. [102], Ziems et al. [104], Ma et al. [103]

Political Science and Economy

Argyle et al. [29], Horton [105], Ziems et al. [104]

Social Science

Social Simulation

Social Simulacra [80], Generative Agents [20], SocialAI
School [108], AgentSims [34], S3 [78], Williams et al. [109], Li
et al. [106], Chao et al. [107]

Jurisprudence

ChatLaw [111], Blind Judgement [112]
...
Ziems et al. [104], Bail et al. [113]

ChemCrow [76], ChatMOF [115], Boiko et al. [114]

Natural Science

Experiment Assistant

ChemCrow [76], Boiko et al. [114], Grossmann et al. [121]

Natural Science Education

ChemCrow [76], CodeHelp [119], Boiko et al. [114], MathA-
gent [116], Drori et al. [117], EduChat [87], FreeText [120]

CS & SE

RestGPT [71], Self-collaboration [24], SQL-PALM [89],
...
ProAgent [131], LLM4RL [132], PET [133], REMEM-
...
create LLM agents that can adapt to specific tasks

based on the information provided by digital twins.
...
more efficient reinforcement learning agents for
...
38, 79, 132�135, 137�140]. These efforts focus on
...
velopers can quickly implement and evaluate agents

according to their customized requirements [19, 82,
...
LangChain enables efficient and cost-effective soft-
...
velopment tasks. SmolModels [126] offers a family
of compact language models suitable for various
tasks. AGiXT [144] is a dynamic AI automation
platform that efficiently manages instructions and
...
tal application that leverages LLMs to efficiently
...
4 LLM-based Autonomous Agent Eval-

uation

Similar to LLMs themselves, evaluating the effec-
tiveness of LLM-based autonomous agents is a chal-
lenging task. This section outlines two prevalent
approaches to evaluation: subjective and objective
...
tates that human evaluators differentiate between
...
costs, inefficiency, and population bias. To address
...
(1) Task success
resentative evaluation metrics.

metrics: These metrics measure how well an agent
...
tioned metrics used to evaluate the agent effective-
ness, these metrics aim to assess the efficiency of
...
insights into agents� higher-level social cognition.

(3) Multi-task evaluation: In this method, people
...
the effectiveness of different agent modules in inter-
...
ities of LLM-based agents. WebArena [173] offers a

comprehensive website environment that spans mul-
...
reasoning, safety, and efficiency, when utilizing
...
13 diverse targets from leading platforms.
It of-
...
aspects.
[175] extensively introduces the back-
...
[180] shed light on eval-
uating LLMs, addressing what to evaluate, where

to evaluate, and how to assess their performance
in downstream tasks and societal impact.
[181]
...
can effectively simulate many common roles such
...
world society. Inspired by the above case, maybe

an important problem for agent-based simulation
is how to conduct generalized human alignment,
that is, for different purposes and applications, the
...
can yield substantially different outcomes. This is-
...
In this context, LLMs may display
knowledge.
...
these movies. Without implementing appropriate

strategies, LLMs may make decisions based on
...
on. Consequently, the efficiency of agent actions is
...
critic: Off-policy maximum entropy deep reinforce-
...
M A, Lacroix T, Rozi�re B, Goyal N, Hambro E, Azhar
F, others . Llama: Open and efficient foundation lan-
guage models. arXiv preprint arXiv:2302.13971, 2023

10. Touvron H, Martin L, Stone K, Albert P, Almahairi A,
...
15. Schick T, Dwivedi-Yu J, Dess� R, Raileanu R, Lomeli
...
John O P, Donahue E M, Kentle R L. Big five inventory.
...
dama T, Deguchi J. Simplyretrieve: A private and
...
Y, Fan L, Anandkumar A. Voyager: An open-ended
...
41. Modarressi A, Imani A, Fayyaz M, Sch�tze H. Ret-
...
52. Yao S, Yu D, Zhao J, Shafran I, Griffiths T, Cao Y,
...
62. Huang W, Xia F, Xiao T, Chan H, Liang J, Flo-
...
75. Sur�s D, Menon S, Vondrick C. Vipergpt: Visual infer-
...
D, Vosoughi S. Training socially aligned language
...
88. Deng X, Gu Y, Zheng B, Chen S, Stevens S, Wang
...
97. Colas C, Teodorescu L, Oudeyer P Y, Yuan X, C�t�
...
100. Zhuge M, Liu H, Faccio F, Ashley D R, Csord�s R,
...
108. Kova?c G, Portelas R, Dominey P F, Oudeyer P Y. The
socialai school: Insights from developmental psychol-
ogy towards artificial socio-cultural agents.
arXiv
...
predicting and generating metal-organic frameworks.
...
115. Kang Y, Kim J. Chatmof: An autonomous ai system for
...
143. al. e E. Ai-legion.

https://github.com/

eumemic/ai-legion, 2023
...
2312�2323
Jalil S, Rafi S, LaToza T D, Moran K, Lam W. Chatgpt
...
165. Chalamalasetti K, G�tze J, Hakimov S, Madureira B,
...
evaluating how llms feel using emotionbench. arXiv
...
179. Mialon G, Dess� R, Lomeli M, Nalmpantis C, Pasunuru
R, Raileanu R, Rozi�re B, Schick T, Dwivedi-Yu J,
...
183. Zhuo T Y, Li Z, Huang Y, Shiri F, Wang W, Haffari G,
