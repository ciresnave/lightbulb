...
arti?cial intelligence. Neural networks are not, in general, capable of this and it
...
scalable and effective by solving a set of classi?cation tasks based on the MNIST
...
Achieving arti?cial general intelligence requires that agents are able to learn and remember many
different tasks Legg and Hutter [2007]. This is particularly dif?cult in real-world settings: the
...
Continual learning poses particular challenges for arti?cial neural networks due to the tendency for
...
1999, McCloskey and Cohen, 1989, McClelland et al., 1995, Ratcliff, 1990], occurs speci?cally when
...
to support continual learning thus remains a key barrier to the development of arti?cial general
intelligence.

In marked contrast to arti?cial neural networks, humans and other animals appear to be able to learn
...
are critical to retention of task performance. Together, these experimental ?ndings�together with
neurobiological models [Fusi et al., 2005, Benna and Fusi, 2016]�suggest that continual learning
in the mammalian neocortex relies on a process of task-speci?c synaptic consolidation, whereby
...
In this work, we demonstrate that task-speci?c synaptic consolidation offers a novel solution to the
continual learning problem for arti?cial intelligence. We develop an algorithm analogous to synaptic
consolidation for arti?cial neural networks, which we refer to as elastic weight consolidation (EWC
...
arti?cial neural networks by constraining important parameters to stay close to their old values. In
this section we explain why we expect to ?nd a solution to a new task in the neighbourhood of an
older one, how we implement the constraint, and ?nally how we determine which parameters are
important.
...
projections, to optimize performance. Many con?gurations of ? will result in the same performance
[Nielsen, 1989, Sussmann, 1992]; this is relevant for EWC: over-parameterization makes it likely that
there is a solution for task B, ??
A. While
...
B, that is close to the previously found solution for task A, ??

In order to justify this choice of constraint and to de?ne which weights are most important for a task,
it is useful to consider neural network training from a probabilistic perspective. From this point of
view, optimizing the parameters is tantamount to ?nding their most probable values given some data
D. We can compute this conditional probability p(?|D) from the prior probability of the parameters
p(?) and the probability of the data p(D|?) by using Bayes� rule:

log p(?|D) = log p(D|?) + log p(?) ? log p(D)
(1)
Note that the log probability of the data given the parameters log p(D|?) is simply the negative of the
loss function for the problem at hand ?L(?). Assume that the data is split into two independent parts,
one de?ning task A (DA) and the other task B (DB). Then, we can re-arrange equation 1:
...
entire dataset, while the right hand side only depends on the loss function for task B log p(DB|?).
...
on task A (gray) and on task B (cream). After learning the ?rst task, the parameters are at ??
...
for task A. On the other hand, if we constrain each weight with the same coef?cient (green arrow) the restriction
imposed is too severe and we can only remember task A at the expense of not learning task B. EWC, conversely,
?nds a solution for task B without incurring a signi?cant loss on task A (red arrow) by explicitly computing how
...
p(?|DA). This posterior probability must contain information about which parameters were important
...
the posterior as a Gaussian distribution with mean given by the parameters ??
...
be computed from ?rst-order derivatives alone and is thus easy to calculate even for large models,
and (c) it is guaranteed to be positive semi-de?nite. Note that this approach is similar to expectation
...
where LB(?) is the loss for task B only, ? sets how important the old task is compared to the new one
...
each task, we trained the neural network in the traditional way, namely by shuf?ing the data and
processing it in small batches. After a ?xed amount of training on each task, however, we allowed no
...
[Srivastava et al., 2013, Goodfellow et al., 2014]. For each task, we generated a ?xed, random
permutation by which the input pixels of all images would be shuf?ed. Each task was thus of equal
dif?culty to the original MNIST problem, though a different solution would be required for each.
...
two different tasks. At the point at which the training regime switches from training on the ?rst
...
of subsequent tasks. This problem cannot be countered by regularizing the network with a ?xed
...
dropout regularization to EWC. We ?nd that stochastic gradient descent with dropout regularization
...
?xed capacity, we might ask whether it allocates completely separate parts of the network for each
task, or whether capacity is used in a more ef?cient fashion by sharing representation. To assess this,
...
output are indeed being reused for both tasks. This re?ects the fact that the permutations make the
...
continual learning in the classic Atari 2600 task set [Bellemare et al., 2013]. Speci?cally, each
...
with ?xed resources (i.e. network capacity) and has minimal computational overhead.
...
detect and incorporate novel tasks as they are encountered, and allow for rapid and ?exible switching
between tasks [Collins and Frank, 2013]. In the primate brain, the prefrontal cortex is widely viewed
as supporting these capabilities by sustaining neural representations of task context that exert top-
down gating in?uences on sensory processing, working memory, and action selection in lower-level
...
task-speci?c bias and gains at each layer, (d) the full action set in Atari, (e) a task-recognition model,
and (e) the EWC penalty. Full details of hyper-parameters are described in Appendix app:atari. Here
we brie?y describe the two most important modi?cations to the agent: the task-recognition module,
...
weights given by the Fisher information matrix times a scaling factor ? which was optimized by
...
game-speci?c, rather than shared across games. In particular, we allowed each layer of the network
to have biases and per element multiplicative gains that were speci?c to each game.
...
as in [Mnih et al., 2015], the agent never learns to play more than one game and the harm in?icted
...
considered the bene?t to the agent if we explicitly provided the agent with the true task label (Figure
...
targets all weights equally), the inverse Fisher ((F + ?I)?1; blue; i.e. mimicking weight changes allowed by
...
same effect as perturbing in the inverse Fisher space. This suggests that we are over-con?dent about
...
We present a novel algorithm, elastic weight consolidation, that addresses the signi?cant problem
...
models. Speci?cally, [French and Chater, 2002] used random inputs to compute a quadratic approxi-
mation to the energy surface. Their approach is slow, as it requires re-computing the curvature at
...
this low computational complexity by making several simpli?cations, most notably by approximating
...
Laplace approximation) does constitute a signi?cant weakness (see Fig 4C). Our initial explorations
...
(suggestive of updating the weight posterior). While we do not explore the computational bene?ts
...
carry information on the variance [Aitchison and Latham, 2015, P?ster et al., 2010]. The weight for
...
arti?cial intelligence. In this work we show that an algorithm that supports continual learning�which
...
ment: An evaluation platform for general agents. Journal of Arti?cial Intelligence Research, 47:
253�279, 2013.
...
Joseph Cichon and Wen-Biao Gan. Branch-speci?c dendritic ca2+ spikes cause persistent synaptic

plasticity. Nature, 520(7546):180�185, 2015.

Claudia Clopath, Lorric Ziegler, Eleni Vasilaki, Lars B�sing, and Wulfram Gerstner. Tag-trigger-
consolidation: a model of early and late long-term-potentiation and depression. PLoS Comput Biol,
4(12):e1000248, 2008.
...
DC Dowson and BV Landau. The fr�chet distance between multivariate normal distributions. Journal
...
Eric Eaton and Paul L Ruvolo. Ella: An ef?cient lifelong learning algorithm. In International
...
Alex Krizhevsky, Ilya Sutskever, and Geoffrey E Hinton. Imagenet classi?cation with deep convolu-

tional neural networks. In NIPS, pages 1097�1105, 2012.
...
Shane Legg and Marcus Hutter. Universal intelligence: A de?nition of machine intelligence. Minds
...
Jean-Pascal P?ster, Peter Dayan, and M�t� Lengyel. Synapses with short-term plasticity are optimal
...
Mark B Ring. Child: A ?rst step towards continual learning. In Learning to learn, pages 261�292.
...
Rupesh K Srivastava, Jonathan Masci, Sohrob Kazerounian, Faustino Gomez, and Juergen Schmid-
In Advances in Neural Information Processing Systems 26,
huber. Compete to compute.
pages 2310�2318. Curran Associates, Inc., 2013. URL http://papers.nips.cc/paper/
5059-compete-to-compute.pdf.

H�ctor J. Sussmann. Uniqueness of the weights for minimal feedforward nets with a given input-

output map. Neural Networks, 5:589�593, 1992.

Hado van Hasselt, Arthur Guez, and David Silver. Deep reinforcement learning with double q-learning.
Proceedings of the Thirtieth AAAI Conference on Arti?cial Intelligence, pages 2094�2100, 2016.
...
branch-speci?c formation of dendritic spines after learning. Science, 344(6188):1173�1178, 2014.
...
Table 1: Hyperparameters for each of the MNIST ?gures

4 Appendix

4.1 MNIST experiments

We carried out all MNIST experiments with fully-connected networks with recti?ed linear units.
...
was observed to increase for more than ?ve subsequent steps, we terminated this training segment and
...
layers followed by a fully connected layer. The ?rst convolution had kernel size 8, stride 4 and 32
?lters. The second convolution had kernel size 4, stride 2 and 64 ?lters. The ?nal convolution had
kernels size 3, stride 1 and 128 ?lters. The fully connected layer had 1024 units. Note that this
network has approximately four times as many parameters as the standard network, due to having
twice as many fully connected units and twice as many ?lters in the ?nal convolution. The other
departure from the standard network is that each layer was allowed to have task-speci?c gains and
...
randomly with a uniform number between ?? and ?, with ? set to the square root of the incoming
hidden units (for a linear layer) or set to the area of the kernel times the number of incoming ?lters
(for convolutional layers). Biases and gains were initialized to 0 and 1 respectively.

We used an an (cid:15)-greedy exploration policy, where the probability of selecting random action, (cid:15),
decayed with training time. We kept a different timer for each of the tasks. We set (cid:15) = 1 for 5 � 104
...
games only support a small subset of the 18 possible actions, in order to have a uni?ed network
...
The most signi?cant departure from the published models is the automatic determination of the task.
...
p(c(cid:48), t)?(c, c(cid:48))

(cid:88)

c(cid:48)

?(c, c(cid:48)) = ?(c, c(cid:48))(1 ? ?) + (1 ? ?(c, c(cid:48)))?

where ? is the Kronecker delta function and ? is the probability of switching context. The task context
...
?(c, c(cid:48)) p(c(cid:48), t ? 1)p(o|c, t)
...
be seen as an implementation of the ?at forget me not algorithm described in [Kieran et al., 2016].
The parameter ? is not learnt. Instead we use the result from [Veness et al., 2012] where it is shown
that a time decaying switch rate ? = 1/t guarantees good worst case asymptotic perfmance provided
...
Table 2: Hyperparameters for each of the MNIST ?gures
...
Precisely, we computed two tasks� Fishers, F1 and F2, normalized these to each have unit trace, �F1
and �F2, then computed their Fr�chet distance, a metric on the space of positive-semide?nite matrices
...
(cid:16) �F1 + �F2 ? 2( �F1 �F2)1/2(cid:17)
...
which is bounded between zero and one. We then de?ne the overlap as 1 ? d2, with a value of zero
indicating that the two tasks depend on non-overlapping sets of weights, and a value of one indicating
that F1 = ?F2 for some ? > 0.

13

020000400006000080000100000120000140000crazy_climberEWCSGDsingle game020004000600080001000012000riverraidEWCSGDsingle game050100150200250300350400jamesbondEWCSGDsingle game050100150200250300breakoutEWCSGDsingle game0200040006000800010000gopherEWCSGDsingle game010002000300040005000600070008000kangarooEWCSGDsingle game0500010000150002000025000300003500040000kung_fu_masterEWCSGDsingle game10080604020020fishing_derbyEWCSGDsingle game0200400600800100012001400enduroEWCSGDsingle game3020100102030pongEWCSGDsingle game050001000015000200002500030000350004000045000star_gunnerEWCSGDsingle game020004000600080001000012000demon_attackEWCSGDsingle game10050050100boxingEWCSGDsingle game01000200030004000500060007000asterixEWCSGDsingle game01000020000300004000050000road_runnerEWCSGDsingle game0500010000150002000025000defenderEWCSGDsingle game05101520253035freewayEWCSGDsingle game0200040006000800010000krullEWCSGDsingle game02004006008001000120014001600space_invadersEWCSGDsingle game
