...
effective pruning method, termed Wanda (Pruning by Weights and activations),
...
on a per-output basis. Notably, Wanda requires no retraining or weight update,
...
with the magnitude pruning approach. Given a weight matrix W and input feature activations X, we
...
Magnitude PruningWanda401-13-2-1-3-310240113213310240003-20-3-3002401-13-2-1-3-31021283408334893206401000-1-3-3002WeightsWeight ImportanceWeights and activationsWeight ImportancePruned WeightsPruned Weightsgrouped per layergrouped per outputPublished as a conference paper at ICLR 2024
...
remove weight w1, which may be a good strategy if input features x1 and x2 have similar magnitudes.
...
In this case, we should remove weight w2 instead, because this removal clearly exerts a smaller
influence on the neuron output y than removing weight w1.
...
Pruning Metric. Consider a linear layer with weight W of shape (Cout, Cin). For language models,
this linear layer takes in input activations X with a shape of (N � L, Cin), where N and L are
...
the score for the current weight Wij is defined by:

Sij = |Wij| � ?Xj?2

(1)

where | � | represents the absolute value operator, ?Xj?2 evaluates the ?2 norm of jth features
aggregated across N � L different tokens, and the final score is computed by the product of these two
scalar values. We find that ?2 norm tends to work better than other norm functions (e.g., ?1 and ??)
in measuring activation magnitudes. This is possibly because ?2 norm is generally a smoother metric.
...
within each output neuron. Specifically, for a weight Wij that connects input j to output i inside the
...
Sij = (cid:2)|W|2/diag(cid:0)(XT X + ?I)?1(cid:1)(cid:3)

ij

(3)

Here XT X + ?I in the denominator is the Hessian H for the layer-wise reconstruction problem
and ? is the Hessian dampening factor to avoid the collapse of inverse computation. With careful
inspection, we observe that our metric in Equation 1 is similar to the above when ? is 0 and only the
diagonal elements of the Hessian matrix XT X + ?I are retained. Starting from the pruning metric in
...
In the last reduction step, the diagonal of XT X is diag(?Xj?2
...
Method Weight Update Calibration Data
Magnitude
SparseGPT
Wanda
...
Pruning Metric Sij
|Wij|
(cid:2)|W|2/diag(cid:2)(XXT + ?I)?1(cid:3)(cid:3)
|Wij| � ?Xj?2
...
computation. Overall, our method Wanda (Pruning by Weights and activations) has several attractive
...
can be much faster (seconds) than SparseGPT.
...
Evaluation Dense Fine-tuning 50% 4:8

LoRA Fine-tuning. We enforce a lim-
...
Pruning Metric
Magnitude: |Wij|
SparseGPT: (cid:2)|W|2/diag(H?1)(cid:3)
Wanda: |Wij| � ?Xj?
...
Robustness to Calibration Samples. We vary the
...
than the full inverse hessian H?1 of the local layer-
...
128163264128256#CalibrationSamples7.58.08.59.09.5PerplexityLLaMA-7BSparseGPTWandaPublished as a conference paper at ICLR 2024
...
et al. (2023b) and Elena Voita (2023) have demonstrated the existence of prompt-dependent and
...
Ahmet �st�n, and Sara Hooker. Intriguing properties of quantization at scale. In NeurIPS, 2023. 1
...
Michael Bommarito and Daniel Martin Katz. Gpt
...
In International Conference on Learning Representations, 2018. 9
...
Christoforos Nalmpantis Elena Voita, Javier Ferrando. Neurons in large language models: Dead,
...
Castagn�, Alexandra Sasha Luccioni, Fran�ois Yvon, Matthias Gall�, et al. Bloom: A 176b-
...
0.00.40.50.60.70.8Sparsity020406080AccuracyConvNeXtMagnitude,perlayerMagnitude,peroutput0.00.40.50.60.70.8Sparsity020406080AccuracyConvNeXtWanda,perlayerWanda,peroutput0.00.40.50.60.70.8Sparsity020406080AccuracyDeiTMagnitude,perlayerMagnitude,peroutput0.00.40.50.60.70.8Sparsity020406080AccuracyDeiTWanda,perlayerWanda,peroutput0.00.40.50.60.70.8Sparsity020406080AccuracyConvNeXtMagnitudeWanda0.00.40.50.60.70.8Sparsity020406080AccuracyDeiTMagnitudeWandaPublished as a conference paper at ICLR 2024
...
Dense Pruning Method Weight Update
...
SNIP (Lee et al., 2018)
Unstructured
BERT-LTH (Chen et al., 2020) Unstructured
Unstructured
