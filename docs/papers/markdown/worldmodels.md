...
To handle the vast amount of information that ?ows through
...
Google Brain 2NNAISENSE 3Swiss AI Lab, IDSIA (USI & SUPSI)
...
model and perform fast re?exive behaviours when we face
...
muscles re?exively swing the bat at the right time and loca-
...
2012), an arti?cial agent also bene?ts from having a good
...
on a small search space, while not sacri?cing capacity and
...
the current state of the ?eld. Instead, the goal of this article is
...
In this article, we present a simpli?ed framework that we can
...
More speci?cally, the RNN will model P (zt+1 | at, zt, ht),
where at is the action taken at time t and ht is the hidden
state of the RNN at time t. During sampling, we can adjust
a temperature parameter ? to control model uncertainty, as
done in (Ha & Eck, 2017) � we will ?nd adjusting ? to be
...
The following ?ow diagram illustrates how V, M, and C
interacts with the environment:

Figure 8. Flow diagram of our Agent model. The raw observation
is ?rst processed by V at each time step t to produce zt. The input
...
EncoderzDecoderOriginal Observed FrameReconstructed FrameWorld Models
...
bene?ts. Advances in deep learning provided us with the
tools to train large, sophisticated models ef?ciently, pro-
vided we can de?ne a well-behaved, differentiable loss func-
tion. Our V and M models are designed to be trained ef?-
...
assignment problem is dif?cult.
...
For more speci?c information about the models, training pro-
...
knowledge, our agent is the ?rst known solution to achieve
the score required to solve this task.2

2We ?nd this task interesting because although it is not dif?cult
...
To train our V model, we ?rst collect a dataset of 10,000
random rollouts of the environment. We have ?rst an agent
...
and obtain a mediocre score, CarRacing-v0 de?nes solving as
...
agent actually sees during a rollout. The ?gure below is a
...
Training an agent to drive is not a dif?cult task if we have a
...
satisfactory navigation policy. For this reason, we ?rst want
to test our agent by handicapping C to only have access to V
but not M, so we de?ne our controller as at = Wc zt + bc.
...
4. De?ne Controller (C) as at = Wc [zt ht] + bc.
...
these fast re?exive driving decisions during a car race, the
...
our method is the ?rst reported solution to solve this task.
...
demo, one can override the agent�s actions as well as adjust ? to
...
If our world model is suf?ciently accurate for its purpose,
...
4. De?ne Controller (C) as at = Wc [zt ht].
...
the dream environment and escape from deadly ?reballs
launched by monsters generated by M. Our agent achieved
a score of ? 900 time steps in the virtual environment.

Figure 15. Our agent discovers a policy to avoid hallucinated ?re-
balls. In the online version of this article, the reader can interact
with the environment inside this demo.

Figure 14. Our ?nal agent solving VizDoom: Take Cover.

The agent must learn to avoid ?reballs shot by monsters
...
de?ned to be the number of time steps the agent manages to
...
gredients needed to make a full RL environment. We ?rst
...
temperature ? parameter during the sampling process of
zt+1. By increasing the uncertainty, our dream environment
becomes more dif?cult compared to the actual environment.
The ?reballs may move more randomly in a less predictable
path compared to the actual game. Sometimes the agent
may even die due to sheer misfortune, without explanation.

We ?nd agents that perform well in higher temperature
settings generally perform better in the normal setting. In
fact, increasing ? helps prevent our controller from taking
...
exact number of monsters in the ?rst place, an agent that is
...
dif?cult game. However, in the process of doing so, they
...
governed by the M model never shoots a single ?reball dur-
ing some rollouts. Even when there are signs of a ?reball
forming, the agent will move in a way to extinguish the ?re-
...
who ?y across the sky. For this reason, our world model
...
agent can ef?ciently explore ways to directly manipulate the
...
our agent can easily ?nd an adversarial policy that can fool
our dynamics model � it�ll ?nd a policy that looks good
...
extinguish ?reballs after they are ?red during some rollouts.
...
RL training by ?rst initializing the policy network with the
learned policy, but must subsequently rely on model-free
methods to ?ne-tune this policy in the actual environment.
...
to ignore a ?awed M, or exploit certain useful parts of M
...
To make it more dif?cult for our C model to exploit de?-
...
adjust the temperature parameter ? to control the amount of
...
a monster decides to shoot a ?reball or stay put. While
a single diagonal Gaussian might be suf?cient to encode
...
By making the temperature ? an adjustable parameter of
...
makes it more dif?cult for the C model to ?nd adversarial
policies, increasing it too much will make the virtual envi-
ronment too dif?cult for the agent to learn anything, hence
...
556 with ? = 1.15, increasing ? a notch to 1.30 results in a
...
more sophisticated? In any dif?cult environment, only parts
...
model can be improved and re?ned over time. An iterative
...
enough to solve simple tasks. For more dif?cult tasks, we
need our controller in Step 2 to actively explore parts of the
environment that is bene?cial to improve its world model.
An exciting research direction is to look at ways to incorpo-
rate arti?cial curiosity and intrinsic motivation (Schmidhu-
...
curiosity. By ?ipping the sign of M�s loss function in the
...
and using this model to train a policy. Many concepts ?rst
...
designed to solve dif?cult control problems. Using data
...
them dif?cult to scale up to model a large history of high
...
de?ned, and the observation is relatively low dimensional.
...
ef?cient (Finn et al., 2015; Watter et al., 2015; Finn, 2017).
...
troller, as it offers many bene?ts. For instance, we only need
to provide the optimizer with the ?nal cumulative reward,
...
Clune, 2017) have con?rmed that ES is a viable alternative
...
dif?cult RL tasks from high dimensional pixel inputs (Kout-
...
dream world. This approach offers many practical bene?ts.
...
GPUs in a distributed environment. The bene?t of imple-
...
algorithm to ?ne-tune its policy to maximize an objective
...
task. After all, unsupervised learning cannot, by de?nition,
...
time step by time step, without pro?ting from human-like
...
do not re?ect opinions of our proofreaders and colleagues.
...
mensional pixel images, we ?rst resize each image to 64x64
pixels before and use this resized image as the V Model�s
observation. Each pixel is stored as three ?oating point val-
...
low dimension vectors � and ?, each of size Nz. The latent
vector z is sampled from the Gaussian prior N (�, ?I). In
...
each time step, we ?nd the cutoff approach to more stable
...
mixtures and did not model the correlation ? parameter,
hence z is sampled from a factored mixture of Gaussian
distribution.

When training the MDN-RNN using teacher forcing from
the recorded data, we store a pre-computed set of � and ? for
each of the frames, and sample an input z ? N (�, ?) each
time we construct a training batch, to prevent over?tting our
MDN-RNN to a speci?c sampled z.
...
the 64 core machine had been con?gured to run 16 times
...
policy. DoomRNN is more computationally ef?cient com-
...
hidden states. We tried 2 variations, where in the ?rst varia-
...
increased the temperature slightly and used ? = 1.15 to
...
ulation and domain adaptation to improve ef?ciency of
deep robotic grasping. ArXiv e-prints, September 2017.
URL https://arxiv.org/abs/1709.07857.

Brockman, G., Cheung, V., Pettersson, L., Schneider, J.,
Schulman, J., Tang, J., and Zaremba, W. Openai gym.
ArXiv preprint, June 2016. URL https://arxiv.
org/abs/1606.01540.

Cheang, L. and Tsao, D. The code for facial identity
...
based and data-ef?cient approach to policy search.
...
preprint, May 2016. URL https://arxiv.org/
abs/1605.07127.
...
CSC
2541 Course at University of Toronto, 2016. URL
...
Finn, Chelsea. Model-based rl lecture at deep rl boot-
URL https://youtu.be/
...
tion of topographic regularities in arti?cial neural net-
works. Neural Computation, 22(7):1860�1898, July
2010.
...
Gomez, F. and Schmidhuber, J. Co-evolving recurrent
...
1068009.1068092. URL ftp://ftp.idsia.ch/
...
Graves, Alex. Hallucination with recurrent neural net-
...
blog.otoro.net,
2017b. URL http://blog.otoro.net/2017/
...
Keller, GeorgB., Bonhoeffer, Tobias,
...
Kelley, H. J. Gradient theory of optimal ?ight paths. ARS
...
Matthew Guzdial, Boyang Li, Mark O. Riedl. Game
...
McAllister, R. and Rasmussen, C. Data-ef?cient reinforce-
...
Miikkulainen, R. Evolving neural networks.
...
2015. URL https://www.frontiersin.org/
...
learning with model-free ?ne-tuning. ArXiv preprint, Au-
...
systems for autonomous mental development. Trans.
...
Parker, M. and Bryant, B. Neuro-visual control
...
preprint, May 2017. URL https://pathak22.
github.io/noreward-rl/.

Pi, H., Hangya, B., Kvitsiani, D., Sanders, J., Huang, Z.,
and Kepecs, A. Cortical interneurons that specialize
...
inforcement learning. ArXiv preprint, 2017. URL
...
On using self-supervised fully recurrent neural
learning and
networks for dynamic reinforcement
...
Schmidhuber, J. An on-line algorithm for dynamic re-
...
Schmidhuber, J. Developmental robotics, optimal arti?cial
curiosity, creativity, music, and the ?ne arts. Connection
...
Peters, J., and Schmidhuber, J. Parameter-exploring
Neural Networks, 23(4):551�
policy gradients.
559, 2010.
...
Stanley, Kenneth and Clune, Jeff. Welcoming the era
...
Werbos, P. J. Learning how the world works: Speci?cations
...
?cation. In Proceedings of IEEE/CDC Tampa, Florida,
1989.
