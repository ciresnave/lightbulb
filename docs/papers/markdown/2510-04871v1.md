...
HRM contains four learnable components:
the in-
put embedding f I(�; ?I), low-level recurrent network
fL(�; ?L), high-level recurrent network fH(�; ?H), and
the output head fO(�; ?O). Once the input is embedded,
...
where �y is the predicted output answer, zL and zH are
...
through recursing from both fL and f H,
...
3. zL is very far from converged after one fL evalu-
...
point for both zL and zH in order to leverage the 1-step
...
n evaluations of fL and 1 evaluation of fH. This re-
...
a single feature by only carrying zH across supervi-
...
low-level module fH and one applied rarely as an high-
level module ( fH). This requires twice the number of
parameters compared to regular supervised learning
with a single network.

As mentioned previously, while fL iterates on the la-
tent reasoning feature z (zL in HRM), the goal of fH
is to update the solution y (zH in HRM) given the la-
tent reasoning and current solution. Importantly, since
z ? fL(x + y + z) contains x but y ? f H(y + z) does
...
In doing the oppo-
eralization due to overfitting.
...
eralization.
Sudoku-Extreme uses 1000 shuffling
...
Jones, L., Gomez, A. N., Kaiser, ?., and Polosukhin,
...
Can convolutional neural networks
...
with ?1 = 0.9, ?2 = 0.95, small learning rate warm-
