...
compression (Han et al., 2016) offers a solution.
...
LLM-QAT (Liu et al., 2023b), BitDistiller (Du et al., 2024), OneBit (Xu et al., 2024)
...
LUT-GEMM (Park et al., 2024), GPTQ (Frantar et al., 2023), QuIP (Chee et al., 2023),
...
ZeroQuant (Yao et al., 2022), LLM.int8() (Dettmers et al., 2022), SmoothQuant (Xiao et al., 2023),
RPTQ (Yuan et al., 2023a), OliVe (Guo et al., 2023), OS+ (Wei et al., 2023), LLM-FP4 (Liu et al., 2023a),
OmniQuant (Shao et al., 2024b)

KVQuant (Hooper et al., 2024), KIVI (Liu et al., 2024), WKVQuant (Yue et al., 2024)

Unstructured Pruning
(�4.1)

SparseGPT (Frantar and Alistarh, 2023), Wanda (Sun et al., 2024), SAMSP (Shao et al., 2024a), DSnoT (Zhang et al., 2024),
...
LLM-Pruner (Ma et al., 2023), Shortened LLaMA (Kim et al., 2024), FLAP (An et al., 2024), SliceGPT (Ashkboos et al., 2024),
Sheared LLaMA (Xia et al., 2024)

Semi-structured
Pruning (�4.3)

E-Sparse (Li et al., 2023b), SparseGPT (Frantar and Alistarh, 2023), Wanda (Sun et al., 2024)

Chain-of-Thought
(�5.1.1)

MT-COT (Li et al., 2024b), CoT Prompting (Magister et al., 2023), Fine-tune-CoT (Ho et al., 2023),
...
In-context Learning Distillation (Huang et al., 2022), AICD (Liu, 2024)

Instruction Following
(�5.1.3)

Lion (Jiang et al., 2023), LaMini-LM (Wu et al., 2024), SELF-INSTRUCT (Wang et al., 2023d),
Selective Reflection-Tuning (Li et al., 2024a)

White-box KD (�5.2)

MINILLM (Gu et al., 2024), GKD (Agarwal et al., 2024), TED (Liang et al., 2023)

LPLR (Saha et al., 2023), ASVD (Yuan et al., 2023b), LSAER (Sharma et al., 2024)
...
the Vicuna-Instructions (Zheng et al.,
instance,
...
instructions inspired by various user-focused ap-

plications such as Grammarly, StackOverflow, and
...
and Post-Training Quantization (PTQ). The pri-
mary distinction between the two approaches lies
in whether retraining is needed during quantiza-

QATTrainingDataUpdate(Optional)CalibrationDataUpdate(Optional)(Optional)Weight and ActivationQuantizationPTQTokensChannelsKeysQuantizedKeysTokensChannelsValuesQuantized ValuesKV Cache QuantizationCalibrationData UpdateStructuredPruningUnstructuredPruningSemi-StructuredPruning UVDecomposeTrainingDataAugmentedTrainingDataWhite-box KDBlack-box KDWhite-box TeacherTeacher LLMsStudent LMsBlack-box TeacherQuantizationPruningKnowledge Distillation Low-Rank ApproximationHigh-Precision WeightHigh-Precision ActivationLow-Precision WeightLow-Precision ActivationZero-Value Weight?Optional?Table 1: The performance of various representative LLM quantization methods.
...
Weights Activations KV Cache Wikitext-2
...
Remark 1. While QAT can mitigate quantiza-

tion�s accuracy degradation, retraining demands
...
ture maps within the model. Similar with AWQ,
...
tors,
to accelerate quantized matrix multiplica-
...
"channel" denotes individual dimensions or fea-
...
Unstructured pruning preserves
the pruned
...
assesses the significance of a pruning unit by mea-
...
L0, L1, and L2 regularization) into the loss func-
...
nificance of parameter weights and input fea-

ture norms. E-Sparse incorporates global naive
...
SparseGPT (Frantar and Alis-
sity of LLMs.
tarh, 2023) employs block-wise weight partition-
ing, with each block containing M weights.
It
...
from a large and complex model (i.e.,
teacher
...
pacity for generating explanations. Magister et al.
...
tillation dataset for the student models.
Shrid-
...
Remark 5. Black-Box Distillation uses
the
...
often resulting in higher-level performance im-
...
ternal structure and knowledge representations,
...
matrices U and V (i.e., W ? U V ), where U
is m � k and V is k � n, with k much smaller
than m and n. Recent works try to employ low-
...
prevalent in traditional small models. For exam-
...
ployment
is also a significant area for explo-
...
In IEEE/CVF
low image editing instructions.
...
Saleh Ashkboos, Alexander Borzunov, Torsten
SpQR:
Hoefler, and Dan Alistarh. 2024.
...
friendly outlier-victim pair quantization.
In
...
tion of proprietary large language models.
In
...
Jiuxiang Gu, and Tianyi Zhou. 2024a.
Se-
...
Jiaming Tang, Haotian Tang, Shang
...
Jakub Ad�mek, Eric Malmi, and Aliaksei Sev-
...
Iuri Frosio, and Jan Kautz. 2019.
Importance
...
Misra. 2024. The truth is in there:
Improv-
...
toro, Aditya Gupta, Adri� Garriga-Alonso,
...
dreas Stuhlm�ller, Andrew M. Dai, Andrew
...
Zoph, Bart?omiej Bojanowski, Batuhan �zyurt,
...
Burch, Christopher Waites, Christian Voigt,
...
Levy, Daniel Mosegu� Gonz�lez, Danielle Per-
...
Pavlick, Emanuele Rodol�, Emma Lam, Eric
...
n�ndez Fisac, James B Simon, James Kop-
...
Oliveros-Col�n, Luke Metz, L�tfi Kerem Senel,
...
Htut, Pinyu Hwang, Piotr Mi?kowski, Piyush
Patil, Pouya Pezeshkpour, Priti Oli, Qiaozhu
Mei, Qing Lyu, Qinlang Chen, Rabin Ban-
jade, Rachel Etta Rudolph, Raefer Gabriel,
Rahel Habacker, Ramon Risco, Rapha�l Mil-
li�re, Rhythm Garg, Richard Barnes, Rif A.
...
sunori Hashimoto, Te-Lin Wu, Th�o Desbor-
...
Xinran Zhao, Xinyi Wu, Xudong Shen, Yadol-
...
Timoth�e Lacroix, Baptiste Rozi�re, Naman
Goyal, Eric Hambro, Faisal Azhar, Aur�lien
...
Aur�lien Rodriguez, Robert Stojnic, Sergey
...
Yue Wang, Weishi Wang, Shafiq R.

Joty,
and Steven C. H. Hoi. 2021.
Codet5:
...
Guangxuan Xiao, Ji Lin, Micka�l Seznec, Hao
