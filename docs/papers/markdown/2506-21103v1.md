...
components (Wang et al., 2018), such as individual modules (Csord�s et al., 2021; Peroni and
...
scalar gate value for each token position and block in the first half of the model. If the gate at block ?
is zero, we skip the Transformer blocks between ? and L ? ? for the token, and prevent other tokens
from attending to its position in the corresponding self-attention modules.

and Gonz�lez et al. (2025) found that when layers are removed or swapped in pre-trained models, the
...
A standard decoder-only Transformer has L layers or blocks, each of which comprises a self-attention
and FFN module. We denote the input to layer ? at token position i by h(i,?) ? Rd where d is the
model dimension, and the inputs at all token positions i ? 1..N by H(?) ? RN �D.
...
a(i,?) = h(i,??1) + g(i,?) GatedAttn (H(??1), g(?) )

h(i,?) = a(i,?) + g(i,?) FFN(a(i,?)).

The modifications are highlighted . If the gate value g(i,?) ? R for a token position i is zero, then we
...
For each block ? < L/2 in the first half of the model, we introduce a linear layer that outputs a scalar
soft mask value s(i,?) ? 0, which accumulates over the first half of the model:
...
When the accumulated soft mask value S(i,?) ? 1, we skip processing the residual vector at token
position i by the Transformer blocks [?, L ? ?). Hence, for each block ? ? L/2 in the second half of
the model, we use the accumulated soft mask value S(i,L???1) of the opposing block.

The corresponding scalar gate value g(i,?) ? [0, 1] is:

g(i,?) =

(cid:40)

1 ? clamp (cid:0)S(i,?), 0, 1(cid:1)
1 ? clamp (cid:0)S(i,L???1), 0, 1(cid:1)
...
The sparsity of the gate values g(i,?) determines the reduction in the number of active parameters: for
a single token i, it is the number of blocks at which the gate value is zero multiplied by the number
of parameters in a Transformer block NB. With multiple tokens, the reduction is 2NB
?<L/2 z?,
where z? is the sparsity of the gate values before tokens are processed by block ?.
The gating mechanism introduces (d + 1)L/2 parameters w(?) and b(?), i.e., d + 1 for each block
? < L/2 in the first half of the model. If all these parameters are zero, all the gate values equal one
...
This is equivalent to adding ln gj to the pre-softmax attention logits, and can be implemented
straightforwardly as a score modification within the FlexAttention framework (Dong et al., 2024). In
practice, we apply a lower bound of ? = 1 � 10?6 to gj before ln to prevent infinities.
...
Table 1: Alternative techniques to control the sparsity of the gate values. Recall that g? and s2
? are
the mean and variance of the gate values over the token positions in a batch, and �?
? are the
targets at layer ? for the mean and variance of the gate values, respectively. For the techniques with
adaptive coefficients, ?? and ?? are updated by the same algorithm.
...
outputs with greater norms (Lawson et al., 2024; Csord�s et al., 2024a; Kim et al., 2025).

The gating mechanism we propose effectively introduces skip connections between opposing pairs of
Transformer blocks (Section 2.1). Like Csord�s et al. (2024a), who consider a Universal Transformer
...
residual input to and output of the attention and FFN modules:

y = x + Norm(Module(Norm(x))).

This scheme differs from the �peri-layernorm� of Csord�s et al. (2024a), who apply a layer normaliza-
...
The targets at layer ? for the population mean and variance of the gate values over token positions
are �?
? as linearly spaced
values between an initial target �?
? as the
variance of the Bernoulli distribution with p = �?

? , respectively. Except where noted, we choose the mean targets �?

L/2. We choose the variance targets ?2?
? ).

0 and a final target �?
...
We denote the adaptive coefficients for the mean and variance of the gate values at layer ? by ?? and
??, respectively, and initialize the coefficients to zero. The regularization loss is then:
...
multiplier ? = 1�10?3 and tolerance ? = 1�10?2 based on observations in small-scale experiments.
...
mean gates �?
L/2 from 1 to 0, the sparsity increases
...
0 fixed to 1, as we decrease the final target �?
...
2022; Dai et al., 2024). While related methods like Zhang et al. (2022); Csord�s et al. (2024b); Jin
...
Transformers exhibit greater redundancy (Lad et al., 2024; Gonz�lez et al., 2025). These findings
...
posed by Csord�s et al. (2021) modulates a block�s contribution, but still requires the full computation
...
top-k tokens at each block, enforcing a fixed computational budget for the sequence. Our method
...
R�bert Csord�s, Kazuki Irie, and J�rgen Schmidhuber. The Neural Data Router: Adaptive Control
In International Conference on
Flow in Transformers Improves Systematic Generalization.
Learning Representations, October 2021. URL https://openreview.net/forum?id=KBQP4A_
J1K.

R�bert Csord�s, Kazuki Irie, J�rgen Schmidhuber, Christopher Potts, and Christopher D. Manning.
...
R�bert Csord�s, Piotr Pi?ekos, Kazuki Irie, and J�rgen Schmidhuber. SwitchHead: Accelerating
...
Ram�n Calvo Gonz�lez, Daniele Paliotta, Matteo Pagliardini, Martin Jaggi, and Fran�ois Fleuret.
...
Julie Kallini, Shikhar Murty, Christopher D. Manning, Christopher Potts, and R�bert Csord�s. MrT5:
...
Pit Neitemeier, Bj�rn Deiseroth, Constantin Eichenberg, and Lukas Balles. Hierarchical Autoregres-
...
Guilherme Penedo, Hynek Kydl�?cek, Loubna Ben allal, Anton Lozhkov, Margaret Mitchell, Colin
...
A. Beygelzimer, F. d'Alch�-Buc, E. Fox, and R. Garnett, editors, Advances in Neural Information
