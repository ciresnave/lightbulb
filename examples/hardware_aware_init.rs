...
use lightbulb::engine::slot_monitor::{SlotPoolMonitor, AdjustmentConfig};
...
fn main() -> anyhow::Result<()> {
    ...
    println!("  Weights: {:.1} GB (f16)", model_profile.weights_bytes as f64 / 1e9);
    ...
    println!("✓ SlotPool initialized with {} slots", config.slot_pool_size);
    ...
    if config.enable_monitoring {
        ...
        let _monitor = SlotPoolMonitor::with_config(
            model_profile.clone(),
            config.dtype_bytes,
            monitor_config,
        );
        println!("✓ SlotPoolMonitor initialized");
        println!("  - Target utilization: {:.0}%", config.memory_utilization_target * 100.0);
        println!("  - Runtime adjustment: enabled");
    }

    println!("\n📈 Chunk Size Configuration:");
    println!("  Chunk size: {} tokens", config.chunk_size);
    println!("  Strategy: {}", if config.hardware.gpu.is_some() {
    ...
    println!("  Prefill throughput: {} tokens/chunk", prefill_tokens_per_chunk);
    println!("  Decode latency: ~{:.0} ms/token (estimated)", decode_latency_ms);
    ...
    println!("  Estimated throughput: {:.0} tokens/sec (decode phase)", throughput_estimate);
    ...
    if improvement > 0.0 {
        ...
        println!("  Adjustment:             {:.0}% (preventing OOM)", improvement);
    ...
    if config.hardware.gpu.is_some() {
        ...
        println!("  • Larger chunk sizes ({}+) work well on GPU", config.chunk_size);
    } else {
        println!("  • CPU mode: Chunk size optimized for L3 cache ({})", config.chunk_size);
    ...
    if stats.utilization_percent < 60.0 {
        ...
        println!("  • Memory headroom available: ~{} additional slots possible", headroom_slots);
    }
