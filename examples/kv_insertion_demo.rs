...
fn main() {
    ...
    println!(
        "Retrieved agriculture context: {} tokens",
        agriculture_context.len()
    );
    ...
    println!(
        "Total cached tokens: {}",
        cache_builder.get_position(slot_id)
    );
    println!(
        "Inserting {} tokens at position {} (after agriculture, before renewables)",
        agriculture_context.len(),
        insertion_pos
    );
    ...
    match cache_builder.insert_context_at(slot_id, insertion_pos) {
        Ok(eviction_result) => {
            ...
            println!(
                "  Positions evicted: {} ranges",
                eviction_result.positions_evicted.len()
            );
            ...
            println!(
                "Need to process: {} tokens (KV-only)",
                full_seq.len() - reprocess_start
            );

            let overhead_pct =
                ((full_seq.len() - reprocess_start) as f32 / full_seq.len() as f32) * 100.0;
